...
PORTA = 5001
ARQUIVO = "dados_b.json"
SERVIDOR_A = "http://localhost:5000"
...
def sincronizar():
    try:
        resposta = requests.get(f"{SERVIDOR_A}/produtos", timeout=3)
...
def cadastrar():
    ...
    try:
        requests.post(
            f"{SERVIDOR_A}/replicar",
            json=dados,
            timeout=2
        )
    except:
        print("Servidor A indisponível")
...
def deletar(id):
    ...
    try:
        requests.delete(f"{SERVIDOR_A}/replicar/{id}", timeout=2)
    except:
        pass
